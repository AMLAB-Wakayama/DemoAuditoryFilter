...
%	23 Jan  2017 (freqz 2^nextpow2() for consistency to C version. (RMS differene is only about 0.03%).)
%
...
for nch = 1:NumCh,
  ...
  if strcmp(SwNorm,'peak') == 1,  % peak gain normalization
     ...
     % for consistency to C version  23 Jan 2017
     [frsp freq] = freqz(GC(nch,1:LenGC(nch)),1,2^nextpow2(LenGC(nch)),SR);
